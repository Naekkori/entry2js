...
try {
    ...
    const objectId = workerData.objectId; // 오브젝트가 아니면 undefined가 됩니다.
    const projectFunctions = workerData.projectFunctions; // 프로젝트 함수 목록
    ...
    const ast = test_ast(script, functionId, objectId, projectFunctions); // 함수 ID, 오브젝트 ID, 프로젝트 함수 목록을 AST 생성기에 전달

    // 코드 생성
    parentPort.postMessage({ status: 'progress', message: '코드 생성 중...' });
    const generatedCode = codeGen(ast, objectId, projectFunctions);
