...
const createWindow = () => {
    if (loadingWindow) {
        loadingWindow.webContents.send('update-message', '메인 창 생성 중...');
    }
    // 새로운 브라우저 창을 생성합니다.
    ...
    if (loadingWindow) {
        loadingWindow.webContents.send('update-message', 'UI 로딩 중...');
    }
    ...
    win.once('ready-to-show', () => {
        if (loadingWindow) {
            loadingWindow.webContents.send('update-message', '완료!');
            setTimeout(() => {
                if (loadingWindow) {
                    loadingWindow.close();
                }
...
const createLoadingWindow = () => {
    ...
    loadingWindow.loadFile('www/loading.html');
...
app.whenReady().then(() => {
    ...
    loadingWindow.webContents.once('did-finish-load', () => {
        loadingWindow.webContents.send('update-message', '초기화 중...');
    });
    createWindow();
